...
const PROGRAM_ID = new PublicKey("BLBysPbLSYXUjwXY3hJhjLqUwAJ8F3i6Yw7C2ZzA1otp");
...
export default async function discoverPools() {
    ...
    for (const sig of signatures) {
        ...
        for (const balance of tx.meta.preTokenBalances) {
            ...
            if (!vaultsByOwner.has(owner))
                vaultsByOwner.set(owner, []);
            vaultsByOwner.get(owner).push({ vault, mint: balance.mint });
        }
        for (const [owner, vaults] of vaultsByOwner) {
            if (vaults.length < 2 || pools.has(owner))
                continue;
            const ownerIndex = keys.staticAccountKeys.findIndex((k) => k.toBase58() === owner);
            if (ownerIndex !== -1 && ownerIndex < 2)
                continue;
            const solVault = vaults.find((v) => v.mint === SOL_MINT)?.vault;
            const usdcVault = vaults.find((v) => v.mint === USDC_MINT)?.vault;
...
//# sourceMappingURL=discoverPools.js.map