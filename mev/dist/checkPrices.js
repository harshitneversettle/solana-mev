...
export default async function checkPrices() {
    ...
    for (const [poolAddr, vaults] of pools) {
        try {
            const solBal = await connection.getTokenAccountBalance(new PublicKey(vaults.solVault));
            const usdcBal = await connection.getTokenAccountBalance(new PublicKey(vaults.usdcVault));
            if (!solBal.value.uiAmount || !usdcBal.value.uiAmount)
                continue;
            const price = usdcBal.value.uiAmount / solBal.value.uiAmount;
            prices.set(poolAddr, price);
            console.log(`   ${poolAddr}...: ${price.toFixed(4)} USDC/SOL`);
        }
        catch (error) {
    ...
    if (prices.size < 2) {
        console.log("   Need 2+ pools for arbitrage");
        return;
    }
    ...
    if (spreadPercent > 1.0) {
        ...
        console.log(`   Buy:  ${cheapPool} at ${cheapPrice.toFixed(4)}`);
        console.log(`   Sell: ${expensivePool} at ${expensivePrice.toFixed(4)}`);
        console.log(`   Profit: ${spread.toFixed(4)} USDC/SOL`);
    }
}
export { prices };
//# sourceMappingURL=checkPrices.js.map